...
def fetch_historical_candles(pair, hours=6):
    """Fetch historical OHLC candles from Kraken. Designed to run in a daemon thread."""
    try:
        ...
        if data.get("result"):
            ...
            log("Loaded "+str(len(candles))+" historical candles for "+pair)
            return candles
    except Exception as e:
        log("Historical candles fetch error: "+str(e), "WARN")
    return []
...
def start_background_loops():
    ...
    def price_loop():
        # Pre-populate price_history with historical candles in a non-blocking daemon thread
        def preload_history():
            try:
                init_pair = state.get("pair", "SOL/USDC")
                hist = fetch_historical_candles(init_pair)
                if hist:
                    existing = state.get("price_history", [])
                    state["price_history"] = hist + existing
                    if len(state["price_history"]) > 1440:
                        state["price_history"] = state["price_history"][-1440:]
                    log("Preloaded "+str(len(hist))+" historical candles for "+init_pair)
            except Exception as e:
                log("Historical preload warning: "+str(e), "WARN")
