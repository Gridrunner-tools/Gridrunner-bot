...
def fetch_historical_candles(pair, hours=6, timeout_sec=5):
    """Fetch historical OHLC close prices from Kraken as [{time, value}] for chart prefill."""
    try:
        kraken_pair = KRAKEN_PAIRS.get(pair, pair.replace("/","").replace("USDT","USD").replace("USDC","USD"))
        since = int(time.time()) - int(hours * 3600)
        r = requests.get(
            "https://api.kraken.com/0/public/OHLC",
            params={"pair": kraken_pair, "interval": 1, "since": since},
            timeout=timeout_sec
        )
        data = r.json()
        if data.get("error"):
            log("Historical candle API error for "+pair+": "+str(data.get("error")), "WARN")
            return []
        result = data.get("result", {})
        ...
        for k, rows in result.items():
            if k == "last":
                continue
            candles = [{"time": int(p[0]), "value": float(p[4])} for p in rows if int(p[0]) >= cutoff]
            return candles[-1440:]
    except Exception as ex:
        log("fetch_historical_candles error for "+pair+": "+str(ex), "WARN")
    return []
...
def start_background_loops():
    ...
    def price_loop():
        def preload_history_non_blocking():
            try:
                init_pair = state.get("pair","ETH/USDT")
                hist = fetch_historical_candles(init_pair, hours=6, timeout_sec=5)
                if hist:
                    state["price_history"] = hist
                    log("Loaded "+str(len(hist))+" historical candles for "+init_pair)
            except Exception as ex:
                log("historical preload failed: "+str(ex), "WARN")

        threading.Thread(target=preload_history_non_blocking, daemon=True).start()
        while True:
            try:
